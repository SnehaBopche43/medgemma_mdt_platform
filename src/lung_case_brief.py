"""
Lung Cancer MDT Case Brief Generator
...
class LungMDTBriefGenerator:
    ...
    def generate_mdt_brief(self, case_data, imaging_analysis, treatment_recommendations):
        ...
        prompt = f"""You are preparing a comprehensive case brief for a lung cancer MDT meeting.
...
def main():
    """Test the lung MDT brief generator"""
    print("=" * 60)
    print("Testing Lung Cancer MDT Brief Generator")
    print("=" * 60)
    ...
    imaging_path = Path("outputs/evaluations/lung_imaging_analysis_002.json")
    if imaging_path.exists():
        with open(imaging_path, 'r') as f:
            imaging_analysis = json.load(f)
    else:
        print("\nWarning: Imaging analysis not found. Run lung_image_analysis.py first.")
        print("Using placeholder data for demonstration...")
        imaging_analysis = {
            "ai_analysis": "Placeholder imaging analysis - run lung_image_analysis.py to generate actual analysis"
        }
    
    # Load treatment recommendations
    treatment_path = Path("outputs/evaluations/lung_treatment_recommendations_002.json")
    if treatment_path.exists():
        with open(treatment_path, 'r') as f:
            treatment_recommendations = json.load(f)
    else:
        print("\nWarning: Treatment recommendations not found. Run lung_specialist_advisor.py first.")
        print("Using placeholder data for demonstration...")
        treatment_recommendations = {
            "treatment_recommendations": "Placeholder treatment recommendations - run lung_specialist_advisor.py to generate actual recommendations"
        }
    
    # Generate MDT brief
    generator = LungMDTBriefGenerator()
    result = generator.generate_mdt_brief(case_data, imaging_analysis, treatment_recommendations)
    
    print("\nLung Cancer MDT Brief:")
    print("-" * 60)
    ...
    output_path = output_dir / "lung_mdt_brief_002.json"
    ...
    text_output_path = output_dir / "lung_mdt_brief_002.txt"
    with open(text_output_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("LUNG CANCER MDT CASE BRIEF\n")
        f.write("=" * 60 + "\n\n")
