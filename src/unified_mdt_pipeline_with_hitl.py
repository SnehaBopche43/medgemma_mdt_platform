"""
Unified Cancer MDT Pipeline with HITL Approval and References
...
class UnifiedMDTPipeline:
    ...
    def run_pipeline(self, case_file_path, auto_approve=False):
        """
        Run complete MDT pipeline with HITL approval and references
        ...
        print("\n" + "="*80)
        print(f"{config['icon']} {config['display_name'].upper()} MDT PIPELINE WITH HITL & REFERENCES")
        print("="*80 + "\n")
        ...
        try:
            ...
            if auto_approve:
                ...
                self.hitl.approve_recommendation(
        ...
        print("\n" + "="*80)
        print(f"STEP 2: SPECIALIST RECOMMENDATIONS ({config['specialty']})")
        print("="*80 + "\n")
        ...
        try:
            ...
            print(f"   Added {cited_recommendation['reference_count']} references")
    ...
    def _save_results(self, pipeline_results, case_data, config):
        """Save pipeline results with citations"""
        ...
        with open(text_output_file, 'w') as f:
            f.write("="*80 + "\n")
            f.write(f"{config['display_name'].upper()} MDT PIPELINE RESULTS WITH CITATIONS\n")
            f.write("="*80 + "\n\n")
        ...
        print("\n" + "="*80)
        print(f"{config['icon']} PIPELINE COMPLETE WITH HITL & REFERENCES")
        print("="*80)
        ...
        print(f"\nTotal References: {len(pipeline_results['references'])}")
        print("="*80 + "\n")
...
def main():
    ...
    # Test cases for different cancer types
    test_cases = [
    ...
    print("   Testing multiple cancer types with HITL & References")
    print("="*80)
